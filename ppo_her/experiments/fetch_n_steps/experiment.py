from ray import tune
import numpy as np
import os
from copy import deepcopy

''' ----- File Imports ----- '''
from ppo_her.base.experiment import train
from ppo_her.base.config import DEFAULT_CONFIG
from ppo_her.base.run import run
from ppo_her.base.utils import get_experiment_name

if __name__ == '__main__':
    
    # Constants
    NUM_SAMPLES = 3
    
    # Experiment name
    experiment_name = get_experiment_name(__file__)
    print('Running experiment: ' + str(experiment_name) + '_2')

    # Configs
    config = deepcopy(DEFAULT_CONFIG)
    config['total_timesteps'] = int(1000E3)
    config['model_config']['type'] = 'ppo'
    config['model_config']['her_method'] = 'final'
    config['model_config']['stop_experiment_on_success'] = False
    # config['model_config']['policy_kwargs'] = {
    #     'log_std_init': np.log(0.6)
    # }

    config['model_config']['use_her'] = tune.grid_search([True, False])
    config['model_config']['l2_coeff'] = tune.grid_search([0.001, 0.0001, 0.00001, 0.01])
    config['model_config']['n_steps'] = tune.grid_search([2048, 4096, 8192, 16384, 32768, 65536])
    config['env_type'] = tune.grid_search(['FetchReach-v2', 'FetchPush-v2', 'FetchSlide-v2', 'FetchPickAndPlace-v2'])
    '''
    config['model_config']['use_her'] = True
    config['model_config']['l2_coeff'] = 0.001
    config['env_type'] = 'FetchReach-v2'
    '''

    # Run
    PARALLEL = True
    if PARALLEL:
        config['model_config']['experiment_number'] = tune.grid_search(np.arange(NUM_SAMPLES))
        run(
            train, num_samples=1,
            config=config, num_concurrent=None,
            num_nodes=None,
            experiment_name=experiment_name,
        )
    else:
        train(config)

